...
const updateBodyCompositionSchema = z.object({
  body: z.object({
    height: z.number().positive('身高必須大於 0'),
    weight: z.number().positive('體重必須大於 0'),
    age: z.number().int('年齡必須是整數').positive('年齡必須大於 0'),
...
const updateGoalSchema = z.object({
  body: z.object({
    ...
    targetWeight: z.number().positive('目標體重必須大於 0').optional(),
    targetDate: z.string().datetime('目標日期格式不正確').optional(),
    targetFatRate: z.number().min(0).max(100).optional(),
    targetFatWeight: z.number().positive('目標脂肪重必須大於 0').optional(),
    targetMuscleRate: z.number().min(0).max(100).optional(),
    targetMuscleWeight: z.number().positive('目標肌肉重必須大於 0').optional(),
  }),
});
